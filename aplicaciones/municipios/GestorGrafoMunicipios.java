...
public class GestorGrafoMunicipios {
    ...
    public ListaConPI<Municipio> caminoMinimo(Municipio mOrig, Municipio mDst) {
        Integer origen = dicMunis.recuperar(mOrig);
        Integer destino = dicMunis.recuperar(mDst);
        ListaConPI<Integer> listaIndices = grafo.caminoMinimo(origen, destino);
        ListaConPI<Municipio> caminoMinimo = new LEGListaConPI<Municipio>();
        for(listaIndices.inicio(); !listaIndices.esFin(); listaIndices. siguiente()){
            caminoMinimo.insertar(munis[listaIndices.recuperar()]);
        }
        return caminoMinimo;
    }
}
