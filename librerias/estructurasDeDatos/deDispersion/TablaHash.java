...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    public final double factorCarga() {
        return talla*1.0/elArray.length;
    }
    ...
    protected final void rehashing() {
        ...
        int capacidad = siguientePrimo(aux.length*2);
        elArray = new LEGListaConPI[capacidad];
        for (int i = 0; i < elArray.length; i++) { 
            elArray[i] = new LEGListaConPI<EntradaHash<C, V>>();
        }
        ...
        for(int i = 0; i < aux.length; i++){
            aux[i].inicio();
            while(!aux[i].esFin()){
                EntradaHash<C, V> e = aux[i].recuperar();
    ...
    public ListaConPI<C> claves() {
        ...
        for(int i = 0; i < elArray.length; i++){
            ...
            while(!elArray[i].esFin()){
                EntradaHash<C, V> e = elArray[i].recuperar();
    ...
    public final double desviacionTipica() {
        double lMedia = (double)talla / (double)elArray.length;
        double suma = 0.0;
        for (int i = 0; i < elArray.length; ++i) {
            double di = (double)elArray[i].talla() - lMedia;
            suma += di * di;
        }
        return Math.sqrt(suma / (double)elArray.length);
    }
    ...
    public ListaConPI<C> clavesEnLongitud(int n){
        ...
        int cubeta = 0;
        while(!encontrado && cubeta < elArray.length){
            if(elArray[cubeta].talla() == n) encontrado = true;
            else cubeta++;
        }
        if(encontrado){
            ListaConPI<EntradaHash<C, V>> lc = elArray[cubeta];
            for(lc.inicio();!lc.esFin();lc.siguiente()){
                lista.insertar(lc.recuperar().clave);
            }
        }
        return lista;
    }
